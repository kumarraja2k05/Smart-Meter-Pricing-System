...
class PricePlanService:
    ...
    def get_last_week_usage(self, smart_meter_id, limit=None):
        readings = self.electricity_reading_service.retrieve_readings_for(smart_meter_id)
        print("Rwading: ", readings)
        if len(readings) < 1:
            return []
        ...
        last_week_reading = list(read for read in readings if read.time >= last_week) 

        if len(last_week_reading) < 1:
            raise Exception("No Readings")

        average = self.calculate_average_reading(last_week_reading)
        time_elapsed = calculate_time_elapsed(last_week_reading)
        consumed_energy = average * time_elapsed
