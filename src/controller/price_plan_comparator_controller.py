...
@router.get(
    ...
    description="View recommended price plans for last week usage",
)
def last_week_usage(smart_meter_id: str = Path(openapi_examples=OPENAPI_EXAMPLES)):
    price_plan_service = PricePlanService(readings_repository)
    print("Starting the paln service")
    cost = price_plan_service.get_last_week_usage(smart_meter_id)
    return cost
